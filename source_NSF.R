...
#Execute query
res <- dbSendQuery(con, "SELECT COUNT(*) AS no_awards, State, Year, SUM(FundsToDate) AS Funding, IF(State = 'PR',1,0) AS PR from funding WHERE State NOT IN('12', 'NA', 'ON') AND Agency LIKE 'NSF' GROUP BY State, Year")
...
png(file="funding_comparison_NSF.png",height=600,width=1000)
...
dev.off()
png(file="funding_byyear_NSF.png",height=600,width=1000)
