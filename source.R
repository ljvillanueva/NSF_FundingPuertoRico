...
#Execute query
res <- dbSendQuery(con, "SELECT COUNT(*) AS no_awards, State, Year, SUM(FundsToDate) AS Funding, IF(State = 'PR',1,0) AS PR from funding WHERE State NOT IN('12', 'NA', 'ON') GROUP BY State, Year")
...
png(file="funding_comparison.png",height=600,width=1000)
...
dev.off()
png(file="funding_byyear.png",height=600,width=1000)
